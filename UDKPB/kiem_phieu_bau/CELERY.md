...
# Áp dụng cho GPU
# Terminal 1: Upload worker - tăng concurrency lên 2 hoặc 3 (vì GPU xử lý nhanh hơn)
celery -A kiem_phieu_bau worker -Q upload_queue --pool=gevent --concurrency=2 --hostname=upload_worker@%h -l info

# Terminal 2: Counting worker 1
celery -A kiem_phieu_bau worker -Q counting_queue --pool=gevent --concurrency=2 --hostname=counting_worker1@%h -l info

# Terminal 3: Counting worker 2  
celery -A kiem_phieu_bau worker -Q counting_queue --pool=gevent --concurrency=2 --hostname=counting_worker2@%h -l info
